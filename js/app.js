...
//=================
//Accordions
let spollers = document.querySelectorAll("._accordion");
let spollersGo = true;
if (spollers.length > 0) {

	function spollerCLick(e) {
		const spoller = e.target.classList.contains('_accordion') ? e.target : e.target.closest('._accordion');
		if (spollersGo) {
			spollersGo = false;

			if (spoller.closest('._accordions').classList.contains('_one')) {
				let curent_spollers = spoller.closest('._accordions').querySelectorAll('._accordion');
